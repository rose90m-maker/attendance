...
SAFETY_VISION_SYSTEM = (
    ...
    "6) 【고소작업】 사다리·사다리차·고소작업대·붐·바스켓·비계·지붕·트럭 적재함·난간 없는 "
    "가장자리 '위' 또는 '안'에 사람이 있으면, 그 장비를 '기계'로만 분류하지 말고 '추락/전도' 위험요인을 "
    "다른 위험요인과 '함께' 반드시 등록하라. 안전대 결속이 확인되지 않으면 추락 위험은 '상'이다.\n"
    "7) 【복수 위험 필수】 한 사진에 위험이 여럿이면 모두 나열하라. 하나만 쓰고 끝내지 말 것. "
    "특히 사람이 명확히 보이면 '추락'과 '보호구 착용 여부'는 서로 별개 항목으로 각각 확인하라 "
    "(추락을 지적했다고 보호구를 생략하지 마라).\n"
